...
class MIDLMarshal(spec: Spec) extends Marshal(spec) {
  override def typename(tm: MExpr): String = toMidlType(tm, None)
  def typename(name: String, ty: TypeDef): String = idMidl.ty(name)

  override def fqTypename(tm: MExpr): String = toMidlType(tm, spec.javaPackage)
  def fqTypename(name: String, ty: TypeDef): String = withPackage(spec.javaPackage, idMidl.ty(name))

  override def paramType(tm: MExpr): String = typename(tm)
  override def fqParamType(tm: MExpr): String = fqTypename(tm)

  override def returnType(ret: Option[TypeRef]): String = ret.fold("void")(ty => toMidlType(ty.resolved, None))
  override def fqReturnType(ret: Option[TypeRef]): String = ret.fold("void")(ty => toMidlType(ty.resolved, spec.javaPackage))
  ...
  private def toMidlType(tm: MExpr, packageName: Option[String]): String = {
